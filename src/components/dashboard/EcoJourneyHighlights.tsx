  ...
  const [highlights, setHighlights] = useState<EcoHighlight[]>([
    {
      ...
      value: "0 km",
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 xs:w-5 sm:w-6 h-4 xs:h-5 sm:h-6">
  ...
  return (
    <div className="rounded-2xl p-2 xs:p-3 sm:p-4 md:p-5 shadow-lg border border-[#475569]/50 bg-[#1E293B]/50">
      <div className="mb-2 xs:mb-3 sm:mb-4">
        <h2 className="text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl font-bold text-[#F1F5F9] mb-1">🌍 Eco Journey Highlights</h2>
        <p className="text-[10px] xs:text-xs sm:text-sm text-[#94A3B8]">See how your EV journey is driving change, saving the planet, and making you a part of the green revolution.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-1.5 xs:gap-2 sm:gap-3">
        {highlights.map((highlight, index) => (
          <motion.div
            ...
            className={`${highlight.bgColor} rounded-xl p-2 xs:p-3 border border-[#475569]/50 relative overflow-hidden`}
          >
            {/* Glowing edge effect */}
            ...
            <div className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center">
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-1.5 xs:mb-2">
                <div 
                  ...
                  className={`w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r ${highlight.color} flex items-center justify-center relative`}
                >
                  {highlight.icon}
                  {/* Pulsing glow effect */}
                  <div className={`absolute inset-0 rounded-full bg-gradient-to-r ${highlight.color} opacity-50 blur-sm animate-pulse`}></div>
                </div>
              </div>
              ...
              <div className="font-medium text-[#F1F5F9] mb-1 text-[10px] xs:text-xs">{highlight.title}</div>
              <div className="text-[8px] xs:text-[10px] text-[#94A3B8]">{loading ? 'Loading...' : highlight.description}</div>
            </div>
          </motion.div>
        ))}
      </div>
