// Full seed script for all metro cities
...
async function seedDatabase() {
  let client;
  try {
    console.log('🌱 Seeding MongoDB Atlas database with full dataset...');
    ...
    for (const city of metroCities) {
      for (const station of city.stations) {
        ...
        totalStations++;
        totalSlots += station.slots.length;
      }
    }
    
    console.log(`✅ Database seeded successfully!`);
    console.log(`   - ${totalStations} stations across 8 metro cities`);
    console.log(`   - ${totalSlots} charging slots`);
    
    // Verify data was inserted
    const stationsCount = await db.collection("stations").countDocuments();
    console.log('📊 Final stations count:', stationsCount);
...
seedDatabase().then(() => {
  ...
  console.log("🚀 The application should now display complete data in the deployed link!");
  process.exit(0);
}).catch((error) => {
